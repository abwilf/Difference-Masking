...
class VisionTransformer(nn.Module):
    ...
    def __call__(self, x, mask = None):
        ...
        *batch_dims, hw, pp3 = x.shape # (7, 540, 768)
        ...
        rkey = jax.random.PRNGKey(seed_value)
        if mask is not None:
            
            mask = mask.reshape([-1])
            seq = seq.reshape([b2 * w2 * self.pooling_ratio ** 2, self.hidden_size]) # (945*4,768)
            for idx in range(mask.shape[0]):
                seq = seq.at[mask[idx]].set(jax.random.normal(rkey, [768], dtype=seq.dtype))
            seq = seq.reshape([b2 * w2, self.pooling_ratio ** 2, self.hidden_size]) # (945,4,768)
            
            ###################################
            # mask = mask.reshape(-1, 2)
            # for idx in range(mask.shape[0]):
            #     seq = seq.at[mask[idx][0],mask[idx][1]].set(jax.random.normal(rkey, [768], dtype=seq.dtype))
            ###################################
            
        else:
            seq_mask = jax.random.randint(rkey, [b2 * w2], minval=0, maxval=4)[..., None]
            for idx in range(seq.shape[0]):
                seq = seq.at[idx,seq_mask[idx]].set(jax.random.normal(rkey, [768], dtype=seq.dtype))
...
def one_hot_pool(do_pool, idx, v, num_segments, real_bsize=None):
    ...
    pointer = lax.select(do_pool, on_true=idx, on_false=jnp.full(idx.shape, -1)) # (1, 945)
    pointer_oh = jax.nn.one_hot(pointer, num_classes=num_segments, dtype=v.dtype) # (1, 945, 945)
...
class MerlotReserve(nn.Module):
    ...
    def setup(self):
        ...
        # self.scale_params_retrain = self.param('contrastive_scales', nn.initializers.ones, (2,))
        ##########
