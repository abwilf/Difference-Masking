...
def preprocess_tvqa(record, config):
    ...
    if config['mask_where'] == "random":
        ...
        if tf.reduce_sum(bbox.values) > 0.0:
            features['masks_info'], maskidx = create_masks_flatten(features[mask_where], config) 
        else:
            features['masks_info'], maskidx = create_masks_flatten(features[mask_where], config, NO_MASK=True) 
