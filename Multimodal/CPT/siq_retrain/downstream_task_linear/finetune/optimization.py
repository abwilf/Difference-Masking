...
def construct_finetuning_train_state(opt_config, model, params, only_state=False):
    ...
    tx_fns = [
        optax.multi_transform({'zero': zero_grads()}, create_mask(params, lambda s: not s.startswith('proj'))),
    ...
    # tx_fns = [optax.multi_transform({'adam': optax.chain(*prev_tx_fns), 'zero': zero_grads()}, create_mask(params, lambda s: not s.startswith('proj')))]
    # tx_fns = [
    #     optax.multi_transform({'adam': scale_by_bfloat16_adam(b1=opt_config.get('beta_1', 0.9),
    #                            b2=opt_config.get('beta_2', 0.98),
    #                            eps=opt_config.get('eps', 1e-6),
    #                            use_bfloat16=opt_config.get('use_bfloat16_adam', True),
    #                            do_bias_correction=opt_config.get('do_bias_correction', True)), 'zero': zero_grads()}, create_mask(params, lambda s: not s.startswith('proj'))),
    #     subtract_old_weights(weight_decay=opt_config['weight_decay_rate'], mask=mask_fn),
    #     optax.add_decayed_weights(weight_decay=opt_config['weight_decay_rate'], mask=mask_fn),
    #     optax.scale_by_schedule(lr_scale_linearwarmup_lineardecay(num_warmup_steps=opt_config['num_warmup_steps'],
    #                                                               num_train_steps=opt_config['num_train_steps'])),
...
def finetune_train_step(state: train_state.TrainState, batch,
    ...
    if len(state.opt_state) == 6:
        updates, nos0 = tx_fns[0].update(updates, state.opt_state[0], state.params)
        updates, nos1 = tx_fns[1].update(updates, state.opt_state[1], None)
        updates, nos2 = tx_fns[2].update(updates, state.opt_state[2], None)
        new_opt_state = [nos1, nos2]
    else:
        # HACK - do the first two things separately
        # assert len(state.opt_state) == 5
        updates, nos0 = tx_fns[1].update(updates, state.opt_state[0], None)
        updates, nos1 = tx_fns[2].update(updates, state.opt_state[1], None)
        # updates, nos2 = tx_fns[2].update(updates, state.opt_state[2], None)
    ...
    # existing params
    # new_opt_state = [nos0, nos1]
    n = 3 if len(state.opt_state) == 6 else 2
    for i in range(3):
        updates, nos = tx_fns[i + 3].update(updates, state.opt_state[i + n], state.params)
        new_opt_state.append(nos)

    # import pdb; pdb.set_trace()
    # new_params_proj = optax.apply_updates(state.params['proj'], updates['proj'])
    new_params = optax.apply_updates(state.params, updates)
    # new_params = state.params.copy({'proj':new_params_proj})
