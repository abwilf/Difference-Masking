...
for item in data:
    question = item.pop('q').lower()
    if question.startswith('why '):
        print('Starting with why: ',question)
        count_why += 1
    elif ' why ' in question:
        print('Middle with why: ',question)

print(len(data), count_why)
...
with GCSTFRecordWriter(out_fn, auto_close=False) as tfrecord_writer:
    for item in data:
        ...
        question = item['q'].lower()
        if not (question.startswith('why ')):
            continue
