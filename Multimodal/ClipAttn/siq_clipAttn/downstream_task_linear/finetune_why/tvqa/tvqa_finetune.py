...
load_dotenv('/home/sakter/merlot_reserve_retrainer/downstream_task/finetune_why/.env')
...
parser.add_argument(
    ...
    help='wandb_name',
    type=str,
    default='merlotreserve-tvqa-finetune',
)
parser.add_argument(
    '--joint_proj',
...
TRAIN_SIZE =  TOTAL_SIZE #config['data']['num_train_files'] * RECORD_PER_FILE if PERCENT_DATASET < 1.0 else TOTAL_SIZE
...
config['_ckpt'] = '/home/sakter/results/retrain/out/base.yaml/'+ext+'/ckpt_'+data_ckpt[args.percent_data] 
# config['_ckpt'] = 'gs://merlotreserve/ckpts/base_resadapt'

if not config['_ckpt'].startswith('gs://'):
    if not os.path.exists(config['_ckpt']):
        print("This checkpoint is not in the directory!")
        sys.exit()

LOG = f"/home/sakter/results/retrain/finetune_why/{ext}.log"    
