...
def preprocess_tvqa(record, config):
    ...
    if config['mask_where'] == "random":
        features['masks_info'], maskidx = create_masks_flatten(features["fmasks"], config, NO_MASK=True)
    else:
        mask_where = config['mask_where'][0]+'masks'
        if tf.reduce_sum(bbox.values) > 0.0:
            features['masks_info'], maskidx = create_masks_flatten(features[mask_where], config) # create_masks(features[mask_where], config)
        else:
            features['masks_info'], maskidx = create_masks_flatten(features[mask_where], config, NO_MASK=True) #create_masks(features[mask_where], config, NO_MASK=True)
...
def create_masks(mask, config, NO_MASK = False):
    ...
    *batch_dims, hw, pp3 = mask.shape
    ...
    if NO_MASK:
        # tf.random.set_seed(seed_value)
        seq_mask = tf.random.uniform([b2 * w2], minval=0, maxval=4, dtype=tf.int32, seed=0)
    ...
    if percent_masks < 0.25:
        ...
        tf.random.set_seed(seed_value)
        seq_mask = tf.random.uniform([get_shape_list(remidx)[0]], minval=0, maxval=4, dtype=tf.int32, seed=0)#[..., None]
...
def create_masks_flatten(mask, config, NO_MASK = False):
    # import pdb; pdb.set_trace()
    *batch_dims, hw, pp3 = mask.shape
    output_grid_h, output_grid_w = config['output_grid']
    h2 = output_grid_h // config['vit_pooling_ratio'] # 9
