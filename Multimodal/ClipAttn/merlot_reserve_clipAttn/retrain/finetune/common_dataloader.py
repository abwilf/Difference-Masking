...
#     fbbox = tf.ragged.stack([x['fbbox'] for x in segment_list])
    ...
    if config['mask_where'] == "random":
        features['masks_info'], maskidx = create_masks_flatten(features["masks"], config, NO_MASK=True)
    else:
...
if __name__ == '__main__':
    ...
    dataset = tf.data.TFRecordDataset(['/data/sakter/preprocessed_clip/out/train245of256.tfrecord'])
    # # For eager debugging
