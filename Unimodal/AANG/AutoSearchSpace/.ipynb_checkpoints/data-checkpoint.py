...
class pretraining_dataset(Dataset):
	...
	def __init__(self, input_file, max_pred_length=80):
...
class DataOptions(object):
	def __init__(self, args, tokenizer, bert_tokenizer, bert_model, data_dict, output_dict, input_file_path):
		...
		self.masks_info = pretraining_dataset(input_file_path)
...
class DataTransformAndItr(object):
	def __init__(self, args, dataoptions, input_tform_dict, output_dict):
		...
		self.masks_info = dataoptions.masks_info
	...
	def get_similarity(self, sent):
		clean_sent_raw = [word.strip('.,') for word in sent.lower().strip('.!').split(" ")]
		clean_sent = [word for word in clean_sent_raw if word not in self.sim_dict]
		if len(clean_sent) > 0:
			input_cytokine = self.dataOpts.tokenizer_bert.batch_encode_plus(clean_sent, add_special_tokens=True, return_tensors="pt", padding=True, truncation=True)
			output_cytokine = self.dataOpts.model_bert(**input_cytokine)
			assert len(clean_sent) == output_cytokine.pooler_output.shape[0]  
            
		sim = []
		new_word_idx = 0
		for word in clean_sent_raw:
			if word in self.sim_dict:
				sim.append(self.sim_dict[word])
			else:
				if self.emb_chem.pooler_output.shape[0] > 1:
					temp_sim = -1000.00
					for sidx in range(self.emb_chem.pooler_output.shape[0]):
						tsim = torch.cosine_similarity(self.emb_chem.pooler_output[sidx][None], output_cytokine.pooler_output[new_word_idx][None]).detach().numpy()[0]
						if temp_sim < tsim:
							temp_sim = tsim
					sim.append(temp_sim)
					self.sim_dict[word] = temp_sim
				else:
					temp_sim = torch.cosine_similarity(self.emb_chem.pooler_output, output_cytokine.pooler_output[new_word_idx][None]).detach().numpy()[0]
					sim.append(temp_sim)
					self.sim_dict[word] = temp_sim                    
				new_word_idx += 1
		...
		sim /= sim.sum()
		K = int(1.0*len(sim)) #int(0.15*len(sim))
	...
	def get_similarity_min(self, sent):
		clean_sent_raw = [word.strip('.,') for word in sent.lower().strip('.!').split(" ")]
		clean_sent = [word for word in clean_sent_raw if word not in self.sim_dict]
		if len(clean_sent) > 0:
			input_cytokine = self.dataOpts.tokenizer_bert.batch_encode_plus(clean_sent, add_special_tokens=True, return_tensors="pt", padding=True, truncation=True)
	...
	def get_similarity_mean(self, sent):
		clean_sent_raw = [word.strip('.,') for word in sent.lower().strip('.!').split(" ")]
		clean_sent = [word for word in clean_sent_raw if word not in self.sim_dict]
		if len(clean_sent) > 0:
			input_cytokine = self.dataOpts.tokenizer_bert.batch_encode_plus(clean_sent, add_special_tokens=True, return_tensors="pt", padding=True, truncation=True)
		...
		for word in clean_sent_raw:
			if word in self.sim_dict:
				...
				if self.emb_chem.pooler_output.shape[0] > 1:
					...
					for sidx in range(self.emb_chem.pooler_output.shape[0]):
						tsim = torch.cosine_similarity(self.emb_chem.pooler_output[sidx][None], output_cytokine.pooler_output[new_word_idx][None]).detach().numpy()[0]
						temp_sim.append(tsim)
					sim.append(sum(temp_sim)/(1.0*len(temp_sim)))
					self.sim_dict[word] = sum(temp_sim)/(1.0*len(temp_sim))
				else:
					temp_sim = torch.cosine_similarity(self.emb_chem.pooler_output, output_cytokine.pooler_output[new_word_idx][None]).detach().numpy()[0]
					sim.append(temp_sim)
					self.sim_dict[word] = temp_sim                    
	...
	def get_masked_indices(self, sent, tokenized_sent):
		if self.mask_selection_process == "max":
			sim_word = self.get_similarity(sent)
		elif self.mask_selection_process == "min":
			sim_word = self.get_similarity_min(sent)
		elif self.mask_selection_process == "mean":
			sim_word = self.get_similarity_mean(sent)
		idx = 1
	...
	def get_masked_indices_supervised(self, idx, tokenized_sent):
		input_ids, segment_ids, input_mask, masked_lm_labels, next_sentence_labels = self.masks_info[idx]
		# input_mask = input_mask[:tokenized_sent.shape[0]]
		# print(input_mask.shape, tokenized_sent.shape)
	...
	def get_masked_indices_token(self, sents, tokenized_sent):
		if self.mask_selection_process == "max":
			sim_word = self.get_similarity(sent)
		elif self.mask_selection_process == "min":
			sim_word = self.get_similarity_min(sent)
			...
			sim_word = self.get_similarity_mean(sent)
		idx = 1
		enc = self.dataOpts.tokenizer.batch_encode_plus(sent, add_special_tokens=False)
		desired_output = []
		masked_tok_indices = []
		masked_tok_dict = {}
		total_len = 0
	...
	def collate(self, examples, token_probas):
		...
		if self.has_alpha:
			...
			for idx, x in enumerate(examples):
				input_id, maksed_tok_index = self.get_masked_indices_supervised(x['idx'], inputs[idx])
				maksed_tok_indices.append(maksed_tok_index)
				input_ids.append(input_id)
			inputs = torch.stack(input_ids)
			maksed_tok_indices = torch.stack(maksed_tok_indices)
            # maksed_tok_indices = torch.stack([self.get_masked_indices_supervised(x['idx'], inputs[idx]) for idx, x in enumerate(examples)])
			# maksed_tok_indices = None
