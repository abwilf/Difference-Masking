...
def get_classifier_samples(dataset, nsamples):
...
class ModelWithAuxTasks(AutoModel):
	...
	def add_transformer_vocab(self, tokenizer, namespace: str = "tokens"):
		"""
	...
	def setup_datasets(self, dataset_split_dict, model_name, max_seq_len, label_vocab=None, lazy=False):
		...
		pretrain_vocab = self.add_transformer_vocab(tokenizer)
		...
		for idx_, fname in dataset_split_dict.items():
			...
			for instance in all_samples:
				...
				sentence = tokens.as_tensor(tokens.get_padding_lengths())['tokens']['token_ids']
