...
class DataTransformAndItr(object):
	...
	# 				tsim = torch.cosine_similarity(self.emb_chem.pooler_output[sidx][None], output_cytokine.pooler_output[idx][None]).detach().numpy()[0]
	# 				if temp_sim < tsim:
	# 					temp_sim = tsim
	# 			sim.append(temp_sim)
	# 		else:
	# 			sim.append(torch.cosine_similarity(self.emb_chem.pooler_output, output_cytokine.pooler_output[idx][None]).detach().numpy()[0])
	# 	sim = np.array(sim)
